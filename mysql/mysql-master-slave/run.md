...
# 登录mysql
mysql -uroot -proot
change master to master_host='IP地址或域名',master_port=3306, master_user='slave', master_password='123456', master_log_file='mysql-bin.000003', master_log_pos= 769, master_connect_retry=30;
...
stop slave;
# 导出主库数据
mysqldump -h IP地址或域名 -P 3306 -uroot -proot --all-databases > /tmp/all.sql
# 导入到从库
mysql -uroot -proot
source /tmp/all.sql;
...
show master status;
# 从库操作
change master to master_host='IP地址或域名',master_port=3306, master_user='slave', master_password='123456', master_log_file='mysql-bin.000004', master_log_pos= 488117, master_connect_retry=30;
start slave;
# 查看主从同步状态
show slave status \G
#              Slave_IO_Running: Yes
